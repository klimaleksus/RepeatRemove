...
function wave_data(){
  return {
    header: Buffer.alloc(sizeof("wave_header")),
    buffer: null,
    length: 0,
    bytes: 0,
    shift: 0,
...
function skip_chunk(
  ...
  var tail = sizeof("wave_header")-(in_header.byteOffset-head.byteOffset);
  var temp;
  temp = malloc(skip);
  if(!temp)
    return false;
...
function print_time(
  ...
  var sec = (sample/rate)|0;
  var min = (sec/60)|0;
  var msc = (1.0*(sample%rate)/rate*100000)|0;
  sec = (sec%60)|0;
  return sprintf(
    "%d:%s.%s",
    min,
...
function seek_boundary(
  wave,
  point_in,
  length_of
){
  var length = wave.length;
  var area = (length/1000)|0;
  var first = point_in;
  var second = point_in+length_of;
  var move_left = 0;
  var move_right = 0;
  var diff_left = -1;
  var diff_right = -1;
...
async function finder_use(
  ...
  var Return = {};
  var i,j;
  var length_from = finder[0].from;
  var length_size = finder[0].size;
  var temp;
  var zone;
  var from;
  var size;
  var radius = (wave.length/2000)|0;
  var diff;
  var best;
  var best_from = 0;
  var best_size = 0;
...
async function find_repeat(
  ...
  var loop_start;
  var loop_middle;
  var seek_from = (length/8)|0;
  var seek_to = (length/2)|0;
  var seek_step = (length/16)|0;
  var seek;
  var seek_curr;
  var reverse;
  var loop_curr;
